...
int esp_readwrite_file(const char *filename, char *rbuf, const char *wbuf, size_t length)
{
        ...
        do {
			...
			if ((!S_ISREG(inode->i_mode) && !S_ISBLK(inode->i_mode))) {
                ...
                if (wbuf) {
                        esp_dbg(ESP_SHOW, "%s: Writing %u bytes to file %s from %p\n", __FUNCTION__,
                                        (unsigned int)length, filename, wbuf);
			if (!(filp->f_mode & FMODE_CAN_WRITE)) {
			    esp_dbg(ESP_DBG_ERROR, "%s: cannot write file %s\n", __FUNCTION__, filename);
			    break;
			}
			if ( (ret=kernel_write(filp, wbuf, length, 0)) < 0) {
                            esp_dbg(ESP_DBG_ERROR, "%s: Write %u bytes to file %s error %d\n", __FUNCTION__,
                                        (unsigned int)length, filename, ret);
                            break;
                        }
                } else if (rbuf) {
                        esp_dbg(ESP_SHOW, "%s: Reading %u bytes from file %s to %p\n", __FUNCTION__,
                                        (unsigned int)length, filename, rbuf);
			if (!(filp->f_mode & FMODE_CAN_READ)) {
			    esp_dbg(ESP_DBG_ERROR, "%s: cannot read file %s\n", __FUNCTION__, filename);
			    break;
			}
			if ( (ret=kernel_read(filp, 0, rbuf, length)) < 0 ) {
...
static u8 pw_max[6] = {0x52, 0x4e, 0x4a, 0x44, 0x40, 0x38};
...
static void update_pw(char *buf, int dec)
{
	int i;
	for (i = 0; i < 6; i++) {
		buf[34+i] = pw_max[i]-dec;
		if (buf[34+i] < 0x28)
			buf[34+i] = 0x28;
	}
}
...
static void fcctest_set(u8 *buf, int size)
{
	...
	esp_dbg(ESP_DBG_TRACE, "%s mode 0x%08x id 0x%02x\n", __func__, mode, fm->id);
	...
	switch (fm->id) {
		...
		case FCC_MODE_CONT_TX:
			...
			update_pw(buf, fm->u.fcc_cont_tx.pw_dec);
			break;
		case FCC_MODE_NORM_TX:
			...
			update_pw(buf, fm->u.fcc_norm_tx.pw_dec);
			break;
		case FCC_MODE_TONE_TX:
			esp_dbg(ESP_SHOW, "tone tx");
                        buf[90] = 0x1;
			buf[82] = fm->u.fcc_tone_tx.channel;
			buf[91] = 0;
			update_pw(buf, fm->u.fcc_tone_tx.pw_dec);
			break;
