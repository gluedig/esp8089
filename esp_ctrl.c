...
int sip_parse_events(struct esp_sip *sip, u8 *buf)
{
	...
	case SIP_EVT_ROC: {
		...
		sprintf(test_res_str, "esp_host: %llx esp_target: %.*s", DRIVER_VER, *len, p);
                esp_dbg(ESP_DBG_TRACE, "%s %s\n", __func__, test_res_str);
...
int sip_send_set_sta(struct esp_pub *epub, u8 ifidx, u8 set, struct ieee80211_sta *sta, struct ieee80211_vif *vif, u8 index)
{
	...
	if(set){
		if(sta->ht_cap.ht_supported){
			...
			if(sta->supp_rates[NL80211_BAND_2GHZ] & (~(u32)CONF_HW_BIT_RATE_11B_MASK)){
